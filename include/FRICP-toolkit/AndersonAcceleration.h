...
public:
	...
	{
		current_u_ = Eigen::Map<const FRICPTypes::VectorX>(u, dim_);
	}

	const FRICPTypes::VectorX& compute(const Scalar* g)
	{
		assert(iter_ >= 0);

		Eigen::Map<const FRICPTypes::VectorX> G(g, dim_);
		...
		else
		{
			...
			FRICPTypes::VectorX new_inner_prod = (prev_dF_.col(col_idx_).transpose() * prev_dF_.block(0, 0, dim_, m_k)).transpose();
	...
	void init(int m, int d, const Scalar* u0)
	{
		...
		theta_.resize(m);
		dF_scale_.resize(m);
		current_u_ = Eigen::Map<const FRICPTypes::VectorX>(u0, d);
		iter_ = 0;
		col_idx_ = 0;
	}

private:
	FRICPTypes::VectorX current_u_;
	FRICPTypes::VectorX current_F_;
	MatrixXX prev_dG_;
	MatrixXX prev_dF_;
	MatrixXX M_;		// Normal equations matrix for the computing theta
	FRICPTypes::VectorX	theta_;	// theta value computed from normal equations
	FRICPTypes::VectorX dF_scale_;		// The scaling factor for each column of prev_dF
