...
private:
    ...
    {
        MatrixNN s (Eigen::AngleAxis<Scalar>(v(0), FRICPTypes::Vector3::UnitX())
                    * Eigen::AngleAxis<Scalar>(v(1), FRICPTypes::Vector3::UnitY())
                    * Eigen::AngleAxis<Scalar>(v(2), FRICPTypes::Vector3::UnitZ()));
...
#pragma omp section
    ...
    double point_to_plane_gaussnewton(const Eigen::MatrixBase<Derived1>& X,
        ...
        FRICPTypes::Vector3 t = Tk.block(0, 3, 3, 1);
        FRICPTypes::Vector3 u = log_T.tail(3);
...
#pragma omp for
        for (int i = 0; i < X.cols(); i++)
        {
            FRICPTypes::Vector3 xi = X.col(i);
            FRICPTypes::Vector3 yi = Y.col(i);
            FRICPTypes::Vector3 ni = norm_y.col(i);
            double wi = sqrt(w_normalized[i]);

            Matrix33 dedR = wi*ni * xi.transpose();
            FRICPTypes::Vector3 dedt = wi*ni;
