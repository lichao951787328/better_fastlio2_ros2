...
pose_estimator::pose_estimator(rclcpp::Node::SharedPtr node) : node_(node) {
    allocateMemory();

    node_->declare_parameter("relo.priorDir", " ");
    node_->declare_parameter("relo.cloudTopic", "/cloud_registered");
    node_->declare_parameter("relo.poseTopic", "/Odometry");
    node_->declare_parameter("relo.searchDis", 10.0);
    node_->declare_parameter("relo.searchNum", 3);
    node_->declare_parameter("relo.trustDis", 5.0);
    node_->declare_parameter("relo.regMode", 5);
    node_->declare_parameter("relo.extrinsic_T", std::vector<double>());
    node_->declare_parameter("relo.extrinsic_R", std::vector<double>());

    priorDir = node_->get_parameter("relo.priorDir").as_string();
    cloudTopic = node_->get_parameter("relo.cloudTopic").as_string();
    poseTopic = node_->get_parameter("relo.poseTopic").as_string();
    cloudTopic_repub = cloudTopic + "repub";  // FIXME: no use
    poseTopic_repub = poseTopic + "repub";  // FIXME: no use
    searchDis = node_->get_parameter("relo.searchDis").as_double();
    searchNum = node_->get_parameter("relo.searchNum").as_int();
    trustDis = node_->get_parameter("relo.trustDis").as_double();
    regMode = node_->get_parameter("relo.regMode").as_int();

    extrinT_ = node_->get_parameter("relo.extrinsic_T").as_double_array();
    extrinR_ = node_->get_parameter("relo.extrinsic_R").as_double_array();
    ...
    subCloud = node_->create_subscription<sensor_msgs::msg::PointCloud2>(
        cloudTopic, 1, std::bind(&pose_estimator::cloudCBK, this, std::placeholders::_1));
    subPose = node_->create_subscription<nav_msgs::msg::Odometry>(
        poseTopic, 500, std::bind(&pose_estimator::poseCBK, this, std::placeholders::_1));
    pubCloud = node_->create_publisher<sensor_msgs::msg::PointCloud2>("/cloud", 1);
    pubPose = node_->create_publisher<nav_msgs::msg::Odometry>("/pose", 1);
    fout_relo.open(priorDir + "relo_pose.txt", ios::out);
    

    subExternalPose = node_->create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>(
        "/initialpose", 500, std::bind(&pose_estimator::externalCBK, this, std::placeholders::_1));
    pubPriorMap = node_->create_publisher<sensor_msgs::msg::PointCloud2>("/prior_map", 1);
    pubPriorPath = node_->create_publisher<sensor_msgs::msg::PointCloud2>("/prior_path", 1);
    pubReloCloud = node_->create_publisher<sensor_msgs::msg::PointCloud2>("/relo_cloud", 1);
    pubInitCloud = node_->create_publisher<sensor_msgs::msg::PointCloud2>("/init_cloud", 1);
    pubNearCloud = node_->create_publisher<sensor_msgs::msg::PointCloud2>("/near_cloud", 1);
    pubMeasurementEdge = node_->create_publisher<visualization_msgs::msg::MarkerArray>("measurement", 1);
    pubPath = node_->create_publisher<nav_msgs::msg::Path>("/path_loc", 1e00000);
...
void pose_estimator::cloudCBK(const sensor_msgs::msg::PointCloud2::SharedPtr msg){
...
void pose_estimator::externalCBK(const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg){
    if(external_flg){
        return ;
    }
...
void pose_estimator::run(){
    ...
    while(rclcpp::ok()){
        ...
        if(idx >= cloudBuffer.size()){
            ...
            publishCloud(pubReloCloud, reloCloud, rclcpp::Time(static_cast<int64_t>(ld_time * 1e9)), "world");
            continue;
        }
...
void pose_estimator::publishThread(){
    ...
    while(rclcpp::ok()){
        rclcpp::spin_some(node_);
        publishCloud(pubPriorMap, priorMap, rclcpp::Time(static_cast<int64_t>(ld_time * 1e9)), "world");
        publishCloud(pubPriorPath, priorPath, rclcpp::Time(static_cast<int64_t>(ld_time * 1e9)), "world");
        publishCloud(pubInitCloud, initCloud, rclcpp::Time(static_cast<int64_t>(ld_time * 1e9)), "world");
        publishCloud(pubNearCloud, nearCloud, rclcpp::Time(static_cast<int64_t>(ld_time * 1e9)), "world");
        rate.sleep();
    }
}
...
void pose_estimator::publish_odometry(const rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr& pubOdomAftMapped){
    odomAftMapped.header.frame_id = "world";
    odomAftMapped.child_frame_id = "body";
    odomAftMapped.header.stamp = rclcpp::Time(static_cast<int64_t>(ld_time * 1e9));
...
void pose_estimator::publish_path(const rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr& pubPath){
    msg_body_pose.header.frame_id = "world";
    msg_body_pose.header.stamp = rclcpp::Time(static_cast<int64_t>(ld_time * 1e9));
    
    path.header.frame_id = "world";
    path.header.stamp = rclcpp::Time(static_cast<int64_t>(ld_time * 1e9));
