...
private:
  ...
  sensor_msgs::msg::Imu::SharedPtr last_imu_;     // 上一帧imu数据
  deque<sensor_msgs::msg::Imu::SharedPtr> v_imu_; // imu队列
...
void ImuProcess::UndistortPcl(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, pcl::PointCloud<PointType> &pcl_out)
{
  ...
  const double imu_beg_time = v_imu.front()->header.stamp.sec + v_imu.front()->header.stamp.nanosec * 1e-9; // 拿到当前帧头部的imu的时间
  const double imu_end_time = v_imu.back()->header.stamp.sec + v_imu.back()->header.stamp.nanosec * 1e-9;  // 拿到当前帧尾部的imu的时间
  ...
  for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++)
  {
    ...
    if (head->header.stamp.sec + head->header.stamp.nanosec * 1e-9 < last_lidar_end_time_) // 前一IMU时间小于上一lidar结束时间
    {
      dt = tail->header.stamp.sec + tail->header.stamp.nanosec * 1e-9 - last_lidar_end_time_; // 记录后一IMU时间与上一lidar结束时间之差
    }
    else
    {
    ...
    double &&offs_t = tail->header.stamp.sec + tail->header.stamp.nanosec * 1e-9 - pcl_beg_time; // 后一个IMU时刻与lidar起始时刻时间差
