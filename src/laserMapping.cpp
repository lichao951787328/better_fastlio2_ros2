...
deque<sensor_msgs::msg::Imu::SharedPtr> imu_buffer; // IMU数据缓存队列
...
nav_msgs::msg::Path path_imu;
geometry_msgs::msg::PoseStamped msg_imu_pose;
geometry_msgs::msg::Quaternion geoQuat_imu;
...
rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pubHistoryKeyFrames; // 发布loop history keyframe submap
...
rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pubLidarPCL;
std::vector<livox_ros_driver2::msg::CustomMsg::SharedPtr> livox_msg;
...
rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pubOptimizedGlobalMap; // 发布最后优化的地图
...
// TODO: 目标检测投影框回调函数 - Darknet ROS not converted yet
/*
void BoxCallback(const darknet_ros_msgs::msg::BoundingBoxes::SharedPtr msg)
{
    box.clear();
    for (int i = 0; i < msg->bounding_boxes.size(); i++)
    {
        if (msg->bounding_boxes[i].class_id == "person" && msg->bounding_boxes[i].probability > 0.6)
        {
            Box boxi;
...
void publish_frame_world_color(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pubLaserCloudColor)
{
    if (p_pre->lidar_type == LIVOX && camera_en)
    {
        if (scan_pub_en) // 是否发布激光雷达数据
        {
            ...
            sensor_msgs::msg::PointCloud2 laserCloudFullResmsg;
            ...
            sensor_msgs::msg::PointCloud2 livox_color_world_msg;
...
{
    rclcpp::Time timeLaserInfoStamp = rclcpp::Time(static_cast<int64_t>(lidar_end_time * 1e9)); // 时间戳
    string odometryFrame = "camera_init";
...
void recontructIKdTree()
{
    if (updateKdtreeCount == kd_step)  
    {
        ...
        updateKdtreeCount = 0;
        RCLCPP_INFO(nh->get_logger(), "Reconstructed  ikdtree ");
...
void correctPoses()
{
    ...
    if (aLoopIsClosed == true)
    {
        ...
        RCLCPP_INFO(nh->get_logger(), "ISMA2 Update");
        aLoopIsClosed = false;
    }
}
...
bool detectLoopClosureDistance(int *latestID, int *closestID)
{
    ...
    *closestID = loopKeyPre;
    ...
    // RCLCPP_INFO(nh->get_logger(), "Find loop clousre frame ");
    return true;
}
...
void loopClosureThread()
{
    ...
    rclcpp::Rate rate(loopClosureFrequency); // 设置回环检测的频率loopClosureFrequency默认为1hz
...
{
    flg_exit = true;
    RCLCPP_WARN(nh->get_logger(), "catch sig %d", sig);
...
// sensor_msgs::msg::PointCloud2格式点云数据的回调函数,将数据引入buffer中
...
void livox_ros_cbk(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
    mtx_buffer.lock(); // 加锁
    scan_count++;
...
// sensor_msgs::msg::Imu格式IMU数据的回调函数,将数据引入buffer中
void imu_cbk(const sensor_msgs::msg::Imu::SharedPtr msg_in)
{
    ...
    double timestamp = rclcpp::Time(msg->header.stamp).seconds(); // IMU时间戳
...
// livox custom转pcl
void LivoxRepubCallback(const livox_ros_driver::msg::CustomMsg::SharedPtr livox_msg_in)
{
    ...
    sensor_msgs::msg::PointCloud2 pcl_msg_out;
    pcl::toROSMsg(pcl_in, pcl_msg_out);
    pcl_msg_out.header.stamp = rclcpp::Time(timebase_ns);
...
bool sync_packages(MeasureGroup &meas)
{
    ...
    if (!lidar_pushed)
    {
        ...
        if (meas.lidar->points.size() <= 1)
        {
            ...
            RCLCPP_WARN(nh->get_logger(), "Too few input point cloud!\n");
        }
        // 最后一个点的时间小于单帧扫描时间的一半
...
void publish_frame_world(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pubLaserCloudFull)
{
    if (scan_pub_en) // 是否发布激光雷达数据
    {
        ...
        // 发布到sensor_msgs::msg::PointCloud2
...
void publish_effect_world(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pubLaserCloudEffect)
{
    ...
    laserCloudFullRes3.header.stamp = rclcpp::Time().fromSec(lidar_end_time);
...
void publish_map(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pubLaserCloudMap)
{
    ...
    laserCloudMap.header.stamp = rclcpp::Time().fromSec(lidar_end_time);
...
void publish_odometry(const rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr pubOdomAftMapped)
{
    ...
    odomAftMapped.header.stamp = rclcpp::Time().fromSec(lidar_end_time);
...
void publish_path_imu(const rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr pubPath){
    msg_imu_pose.header.stamp = rclcpp::Time().fromSec(lidar_end_time);
...
void publish_path(const rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr pubPath)
{
    ...
    msg_body_pose.header.stamp = rclcpp::Time().fromSec(lidar_end_time);
...
{
    rclcpp::Time timeLaserInfoStamp = rclcpp::Time().fromSec(lidar_end_time); // 时间戳
    string odometryFrame = "camera_init";
...
bool saveMapService(fast_lio_sam::save_mapRequest &req, fast_lio_sam::save_mapResponse &res)
{
    ...
    *globalMapCloud += *globalSurfCloud;
    ...
    rclcpp::Time timeLaserInfoStamp = rclcpp::Time().fromSec(lidar_end_time);
    string odometryFrame = "camera_init";
...
#pragma omp parallel for
    ...
    if (effct_feat_num < 1)
    {
        ...
        RCLCPP_WARN(nh->get_logger(), "No Effective Points! \n");
        return;
    }
...
int main(int argc, char **argv)
{
    ...
    "/usb_cam/image_raw");
    nh->declare_parameter<bool>("common.time_sync_en" = nh->declare_parameter<string>("common.camera_topic" = nh->declare_parameter<string>("common.lid_topic", false);
    lid_topic = nh->get_parameter("common.lid_topic").as_string();
    imu_topic = nh->get_parameter("common.imu_topic").as_string();
    camera_topic = nh->get_parameter("common.camera_topic").as_string();
    time_sync_en = nh->get_parameter("common.time_sync_en").as_bool();

    // export path
    rootDir = "");
    savePCD = nh->declare_parameter<bool>("common.savePCD" = nh->declare_parameter<std::string>("common.rootDir", false);
    savePCDDirectory = "");
    saveSCD = nh->declare_parameter<bool>("common.saveSCD" = nh->declare_parameter<std::string>("common.savePCDDirectory", false);
    saveSCDDirectory = "");
    saveLOG = nh->declare_parameter<bool>("common.saveLOG" = nh->declare_parameter<std::string>("common.saveSCDDirectory", false);
    saveLOGDirectory = "");
    map_save_en = nh->declare_parameter<bool>("common.map_save_en" = nh->declare_parameter<std::string>("common.saveLOGDirectory", false);
    pcd_save_interval = -1);

    // preprocess
    p_pre->lidar_type = nh->declare_parameter<int>("preprocess.lidar_type" = nh->declare_parameter<int>("common.pcd_save_interval", LIVOX);
    p_pre->livox_type = LIVOX_CUS);
    p_pre->blind = nh->declare_parameter<double>("preprocess.blind" = nh->declare_parameter<int>("preprocess.livox_type", 0.01);
    p_pre->N_SCANS = 16);
    p_pre->SCAN_RATE = nh->declare_parameter<int>("preprocess.scan_rate" = nh->declare_parameter<int>("preprocess.scan_line", 10);
    p_pre->point_filter_num = 1);
    p_pre->feature_enabled = nh->declare_parameter<bool>("preprocess.feature_extract_enable" = nh->declare_parameter<int>("preprocess.point_filter_num", false);
    p_pre->time_unit = US);

    // camera 
    camera_en = nh->declare_parameter<bool>("camera.camera_en" = nh->declare_parameter<int>("preprocess.time_unit", false);
    nh->declare_parameter<vector<double>>("camera/camera_external", cam_ex, vector<double>());
    nh->declare_parameter<vector<double>>("camera/camera_internal", cam_in, vector<double>());

    // mapping
    gyr_cov = 0.1);
    acc_cov = nh->declare_parameter<double>("mapping.acc_cov" = nh->declare_parameter<double>("mapping.gyr_cov", 0.1);
    b_gyr_cov = 0.0001);
    b_acc_cov = nh->declare_parameter<double>("mapping.b_acc_cov" = nh->declare_parameter<double>("mapping.b_gyr_cov", 0.0001);
    DET_RANGE = 300.f);
    fov_deg = nh->declare_parameter<double>("mapping.fov_degree" = nh->declare_parameter<float>("mapping.det_range", 180);
    extrinsic_est_en = true);
    mappingSurfLeafSize = nh->declare_parameter<float>("mapping.mappingSurfLeafSize" = nh->declare_parameter<bool>("mapping.extrinsic_est_en", 0.2);
    cube_len = 200);
    keyframeAddingDistThreshold = nh->declare_parameter<float>("mapping.keyframeAddingDistThreshold" = nh->declare_parameter<double>("mapping.cube_len", 20.0);
    keyframeAddingAngleThreshold = nh->declare_parameter<float>("mapping.keyframeAddingAngleThreshold", 0.2);
    nh->declare_parameter<vector<double>>("mapping/extrinsic_T", extrinT, vector<double>());
    nh->declare_parameter<vector<double>>("mapping.extrinsic_R", extrinR, vector<double>());

    // ikdtree
    NUM_MAX_ITERATIONS = nh->declare_parameter<int>("ikdtree.max_iteration", 4);
    kd_step = nh->declare_parameter<int>("ikdtree.kd_step", 40);
    recontructKdTree = nh->declare_parameter<bool>("ikdtree.recontructKdTree", false);
    filter_size_map_min = nh->declare_parameter<double>("ikdtree.filter_size_map_min", 0.2);

    // segment
    sensor_height = nh->declare_parameter<float>("segment.sensor_height", 1.5);
    ground_en = nh->declare_parameter<bool>("segment.ground_en", false);
    tollerance_en = nh->declare_parameter<bool>("segment.tollerance_en", false);
    z_tollerance = nh->declare_parameter<float>("segment.z_tollerance", 1.0);
    rotation_tollerance = nh->declare_parameter<float>("segment.rotation_tollerance", 0.2);
    
    // loop clousre
    loopClosureEnableFlag = nh->declare_parameter<bool>("loop.loopClosureEnableFlag", true);
    loopClosureFrequency = nh->declare_parameter<float>("loop.loopClosureFrequency", 1.0);
    historyKeyframeSearchRadius = nh->declare_parameter<float>("loop.historyKeyframeSearchRadius", 10.0);
    historyKeyframeSearchTimeDiff = nh->declare_parameter<float>("loop.historyKeyframeSearchTimeDiff", 30.0);
    historyKeyframeSearchNum = nh->declare_parameter<int>("loop.historyKeyframeSearchNum", 10);
    historyKeyframeFitnessScore = nh->declare_parameter<float>("loop.historyKeyframeFitnessScore", 0.3);

    // visualization
    globalMapVisualizationSearchRadius = nh->declare_parameter<float>("visualization.globalMapVisualizationSearchRadius", 10.0);
    globalMapVisualizationPoseDensity = nh->declare_parameter<float>("visualization.globalMapVisualizationPoseDensity", 10.0);
    globalMapVisualizationLeafSize = nh->declare_parameter<float>("visualization.globalMapVisualizationLeafSize", 1.0);
    visulize_IkdtreeMap = nh->declare_parameter<bool>("visualization.visulize_IkdtreeMap", false);

    // publish
    path_en = nh->declare_parameter<bool>("publish.path_en", true);
    scan_pub_en = nh->declare_parameter<bool>("publish.scan_publish_en", true);
    dense_pub_en = nh->declare_parameter<bool>("publish.dense_publish_en", true);
    scan_body_pub_en = nh->declare_parameter<bool>("publish.scan_bodyframe_pub_en", true);
    
    paramSetting(); // 设置相机的内参、相机到LiDAR的外参

    // downSizeFilterSurf.setLeafSize(mappingSurfLeafSize = nh->declare_parameter<bool>("publish.scan_bodyframe_pub_en", mappingSurfLeafSize, mappingSurfLeafSize);
    ...
    path_imu.header.stamp = rclcpp::Clock().now();
    ...
    if (p_pre->lidar_type == LIVOX) {
        if (p_pre->livox_type == LIVOX_CUS) {
            auto sub_pcl_livox = nh->create_subscription<livox_ros_driver2::msg::CustomMsg>(
                lid_topic, 200000, livox_pcl_cbk);
        } else {
            auto sub_pcl = nh->create_subscription<sensor_msgs::msg::PointCloud2>(
                repub_topic, 200000, livox_ros_cbk);
        }
    } else {
        auto sub_pcl = nh->create_subscription<sensor_msgs::msg::PointCloud2>(
    ...
    if (camera_en){
        // image_transport not yet converted - needs manual handling
        // TODO: Convert to ROS2
        RCLCPP_WARN(nh->get_logger(), "Camera support not yet converted to ROS2");
    }
    
    auto pubLaserCloudFull = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/cloud_registered", 100000);           // world系下稠密点云
    auto pubLaserCloudColor = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/cloud_registered_color", 100000);    // world系下稠密彩色点云
    auto pubLaserCloudFull_body = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/cloud_registered_body", 100000); // imu(body)系下稠密点云
    auto pubLaserCloudEffect = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/cloud_effected", 100000);           // no used
    auto pubLaserCloudMap = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/Laser_map", 100000);                   // no used
    ...
    auto pubPathIMU = nh->create_publisher<nav_msgs::msg::Path>("/path_imu", 100000);
    auto pubPathUpdate = nh->create_publisher<nav_msgs::msg::Path>("fast_lio_sam/path_update", 100000); // isam更新后的path

    pubLaserCloudSurround = nh->create_publisher<sensor_msgs::msg::PointCloud2>("fast_lio_sam/mapping/keyframe_submap", 1);      // 发布局部关键帧map的特征点云
    pubOptimizedGlobalMap = nh->create_publisher<sensor_msgs::msg::PointCloud2>("fast_lio_sam/mapping/map_global_optimized", 1); // 发布局部关键帧map的特征点云
    pubLidarPCL = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/livox/lidar_repub", 100000);
    // 发布回环匹配关键帧局部map
    pubHistoryKeyFrames = nh->create_publisher<sensor_msgs::msg::PointCloud2>("fast_lio_sam/mapping/icp_loop_closure_history_cloud", 1);
    // 发布当前关键帧经过回环优化后的位姿变换之后的特征点云
    pubIcpKeyFrames = nh->create_publisher<sensor_msgs::msg::PointCloud2>("fast_lio_sam/mapping/icp_loop_closure_corrected_cloud", 1);
    // 发布回环边,rviz中表现为回环帧之间的连线
    pubLoopConstraintEdge = nh->create_publisher<visualization_msgs::msg::MarkerArray>("/fast_lio_sam/mapping/loop_closure_constraints", 1);

    // saveMap发布地图保存服务
    // TODO: Convert service to ROS2
    // srvSaveMap = nh->create_service<fast_lio_sam::srv::SaveMap>("/save_map", saveMapService);
    // savePose发布轨迹保存服务
    // TODO: Convert service to ROS2
    // srvSavePose = nh->create_service<fast_lio_sam::srv::SavePose>("/save_pose", savePoseService);
    // 回环检测线程
