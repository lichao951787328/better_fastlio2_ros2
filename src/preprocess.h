...
public:
  ...
  void process(const livox_ros_driver2::msg::CustomMsg::SharedPtr &msg, pcl::PointCloud<PointType>::Ptr &pcl_out);
  // 对ros的Msg格式的激光雷达数据进行处理
  void process(const sensor_msgs::msg::PointCloud2::SharedPtr &msg, pcl::PointCloud<PointType>::Ptr &pcl_out);
...
private:
  void livox_handler(const livox_ros_driver2::msg::CustomMsg::SharedPtr &msg); // 用于对Livox激光雷达数据进行处理
  void livoxros_handler(const sensor_msgs::msg::PointCloud2::SharedPtr &msg); 
  void velodyne_handler(const sensor_msgs::msg::PointCloud2::SharedPtr &msg); // 用于对velodyne激光雷达数据进行处理
  ...
  void pub_func(pcl::PointCloud<PointType> &pl, const rclcpp::Time &ct);
