 ...
 *
 * @param msg livox激光雷达点云数据,格式为livox_ros_driver2::msg::CustomMsg
 * @param pcl_out 输出处理后的点云数据,格式为pcl::PointCloud<pcl::PointXYZINormal>
 */
void Preprocess::process(const livox_ros_driver2::msg::CustomMsg::SharedPtr &msg, pcl::PointCloud<PointType>::Ptr &pcl_out)
{
  livox_handler(msg);
 ...
 *
 * @param msg velodyne激光雷达点云数据,格式为sensor_msgs::msg::PointCloud2
 * @param pcl_out 输出处理后的点云数据,格式为pcl::PointCloud<pcl::PointXYZINormal>
 */
void Preprocess::process(const sensor_msgs::msg::PointCloud2::SharedPtr &msg, pcl::PointCloud<PointType>::Ptr &pcl_out)
{
  switch (time_unit)
  {
    case SEC:
...
void Preprocess::livox_handler(const livox_ros_driver2::msg::CustomMsg::SharedPtr &msg)
{
  // 清除之前的点云缓存
  pl_surf.clear();
  pl_corn.clear();
...
void Preprocess::livoxros_handler(const sensor_msgs::msg::PointCloud2::SharedPtr &msg)
{
  /* 清除之前的点云缓存 */
  pl_surf.clear();
  pl_corn.clear();
...
void Preprocess::pub_func(pcl::PointCloud<PointType> &pl, const rclcpp::Time &ct)
{
  pl.height = 1;
  pl.width = pl.size();
